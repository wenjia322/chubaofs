...
func (m *metadataManager) opResetMetaPartitionMember(conn net.Conn,
	...
	for _, learner := range req.NewLearners {
		if !mp.IsExistLearner(learner) {
			err = errors.NewErrorf("[opResetMetaPartitionMember]: learner not exists, learner addr[%v], partitionID= %d, ", learner.Addr, req.PartitionId)
			p.PacketErrorWithBody(proto.OpErr, ([]byte)(err.Error()))
			m.respondToClient(conn, p)
			return err
		}
