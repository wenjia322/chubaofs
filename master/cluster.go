...
func (c *Cluster) addDataPartitionRaftLearner(dp *DataPartition, addPeer proto.Peer, addLearner proto.Learner) (err error) {
	dp.Lock()
	defer dp.Unlock()
	if contains(dp.Hosts, addLearner.Addr) {
		err = fmt.Errorf("addDataPartitionRaftLearner: vol[%v], data partition[%v] has contains host[%v]", dp.VolName, dp.PartitionID, addLearner.Addr)
		return
	}
